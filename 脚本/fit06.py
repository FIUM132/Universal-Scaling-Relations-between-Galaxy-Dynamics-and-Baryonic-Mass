...
if __name__ == "__main__":
    ...
    def linear_fit(x, k, b):
        return k * x + b
    # 权重：用R²的倒数作为误差权重，R²越高权重越大
    sigma = 1 / (weights + 1e-6)
    popt, pcov = curve_fit(linear_fit, log_M, log_eta, sigma=sigma)
    ...
    print(f"\n🔍 最终标度关系：η ∝ M_b^{k_scale:.2f} ± {k_err:.2f}")
    print(f"📊 拟合优度R² = {scale_R2:.3f}")
    print(f"📌 有效样本数：n={len(M_b)}")
    ...
    plt.tight_layout()
    plt.savefig("论文级_普适标度关系.png", dpi=150, bbox_inches='tight')
    plt.show()

    print("\n✅ 全部完成！图像已保存为 论文级_普适标度关系.png")